...
namespace Gameboard.Utilities
{
    public class CompanionHandlerUtility : GameboardUtility, ICompanionHandlerUtility
    {
        ...
        #region Dice
        public async Task<CompanionMessageResponseArgs> RollDice(int versionTag, string userId, DieGroup[] dice, int inAddedModifier, string inDiceNotation, string id = null)
        {
        ...
        public async Task<CompanionMessageResponseArgs> SetSelectableDice(int versionTag, string userId, DieGroup[] dice)
        {
            return await SetConfiguration(versionTag, userId, dice, ConfigurationProperty.DiceSelectableDice);
        }
        ...
        void ManageIncomingEvent<T>(IncomingEventDelegate<T> inEvent, EventArgsToGameboard inEventArgs, string inEndpoint)
        {
            ...
            {
                GameboardLogging.LogMessage($"ManageIncomingEventBody: Failed to deserialize incoming event body! Received body was: {inEventArgs.body} using type {typeof(T)}.", GameboardLogging.MessageTypes.Error);
            }
        }
