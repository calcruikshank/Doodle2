    ...
    public class JsonUtility : GameboardUtility, IJsonUtility
    {
        ...
        public JsonUtilityDeserializeResponse<T> DeserializeObject<T>(string inJson)
        {
            try
            {
                ...
                {
                    GameboardLogging.LogMessage($"JsonUtility::DeserializeObject failure for JSON {inJson} - ", GameboardLogging.MessageTypes.Error);
                    GameboardLogging.LogMessage($"{errorEventArgs.errorMessage}", GameboardLogging.MessageTypes.Error);
